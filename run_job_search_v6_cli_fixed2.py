...
3. Applies source patches for known evaluate_job()/helper issues without
   requiring manual edits to the large main script.
"""
...
def _patch_source(source: str) -> str:
    # Fix evaluate_job() referencing location flags before assignment.
    target = (
    ...
    # Fix _decision_reason_parts() requiring keyword-only args even when callers
    # intentionally only pass extra_parts.
    signature_target = (
    ...
    if signature_target in source and signature_replacement not in source:
        source = source.replace(signature_target, signature_replacement, 1)
