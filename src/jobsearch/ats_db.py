...
def init_db(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS workday_target_health (
            company TEXT PRIMARY KEY,
            careers_url TEXT,
        ...
        CREATE TABLE IF NOT EXISTS generic_target_health (
            company TEXT PRIMARY KEY,
            careers_url TEXT,
            empty_streak INTEGER NOT NULL DEFAULT 0,
            success_count INTEGER NOT NULL DEFAULT 0,
    ...
    _add_columns_if_missing(
        ...
        "applications",
        {
            "fit_stars": "fit_stars INTEGER",
            "user_priority": "user_priority INTEGER",
            "tier": "tier INTEGER",
            "score": "score REAL",
            "fit_band": "fit_band TEXT",
            "role_normalized": "role_normalized TEXT",
            "salary_range": "salary_range TEXT",
            "salary_low": "salary_low INTEGER",
            "salary_high": "salary_high INTEGER",
            "salary_text": "salary_text TEXT",
            "status": "status TEXT NOT NULL DEFAULT 'considering'",
            "work_type": "work_type TEXT",
            "compensation_unit": "compensation_unit TEXT",
            "hourly_rate": "hourly_rate REAL",
            "hours_per_week": "hours_per_week REAL",
            "weeks_per_year": "weeks_per_year REAL",
            "normalized_compensation_usd": "normalized_compensation_usd REAL",
            "referral": "referral TEXT",
            "jd_summary": "jd_summary TEXT",
            "notes": "notes TEXT",
            "location": "location TEXT",
            "is_remote": "is_remote INTEGER DEFAULT 0",
            "date_discovered": "date_discovered TEXT",
            "date_added": "date_added TEXT",
            "date_applied": "date_applied TEXT",
            "date_closed": "date_closed TEXT",
            "follow_up_date": "follow_up_date TEXT",
            "follow_up_notes": "follow_up_notes TEXT",
            "resume_version": "resume_version TEXT",
            "cover_letter_notes": "cover_letter_notes TEXT",
            "entry_type": "entry_type TEXT NOT NULL DEFAULT 'application'",
            "resume_url": "resume_url TEXT",
            "cover_letter_url": "cover_letter_url TEXT",
            "resume_tailor_keywords": "resume_tailor_keywords TEXT",
            "resume_tailor_summary": "resume_tailor_summary TEXT",
            "resume_tailor_notes": "resume_tailor_notes TEXT",
            "job_description": "job_description TEXT",
            "prep_company": "prep_company TEXT",
            "prep_why": "prep_why TEXT",
            "prep_tyabt": "prep_tyabt TEXT",
            "prep_questions": "prep_questions TEXT",
            "prep_notes": "prep_notes TEXT",
            "offer_base": "offer_base INTEGER",
            "offer_bonus_pct": "offer_bonus_pct INTEGER",
            "offer_equity": "offer_equity TEXT",
            "offer_signing": "offer_signing INTEGER",
            "offer_pto_days": "offer_pto_days INTEGER",
            "offer_k401_match": "offer_k401_match TEXT",
            "offer_remote_policy": "offer_remote_policy TEXT",
            "offer_start_date": "offer_start_date TEXT",
            "offer_expiry_date": "offer_expiry_date TEXT",
            "offer_notes": "offer_notes TEXT",
            "nego_target_base": "nego_target_base INTEGER",
            "nego_walkaway_base": "nego_walkaway_base INTEGER",
            "nego_market_low": "nego_market_low INTEGER",
            "nego_market_high": "nego_market_high INTEGER",
            "nego_notes": "nego_notes TEXT",
            "matched_keywords": "matched_keywords TEXT",
            "penalized_keywords": "penalized_keywords TEXT",
            "decision_reason": "decision_reason TEXT",
            "description_excerpt": "description_excerpt TEXT",
            "apply_now_eligible": "apply_now_eligible INTEGER NOT NULL DEFAULT 1",
            "source_lane": "source_lane TEXT NOT NULL DEFAULT 'employer_ats'",
            "canonical_job_url": "canonical_job_url TEXT",
            "jd_fingerprint": "jd_fingerprint TEXT",
            "jd_last_changed_at": "jd_last_changed_at TEXT",
            "jd_change_summary": "jd_change_summary TEXT",
            "jd_needs_review": "jd_needs_review INTEGER DEFAULT 0",
            # V2 scoring columns for A/B comparison
            "score_v2": "score_v2 REAL",
            "fit_band_v2": "fit_band_v2 TEXT",
            "v2_canonical_title": "v2_canonical_title TEXT",
            "v2_seniority_band": "v2_seniority_band TEXT",
            "v2_anchor_score": "v2_anchor_score REAL",
            "v2_baseline_score": "v2_baseline_score REAL",
            "v2_flags": "v2_flags TEXT",
            "extraction_method": "extraction_method TEXT",
            "extraction_confidence": "extraction_confidence REAL",
            "last_exported_at": "last_exported_at TEXT",
            "role_cluster": "role_cluster TEXT",
    ...
    _add_columns_if_missing(
        ...
        "email_signals",
        {
            "interview_scheduled_at": "interview_scheduled_at TEXT",
            "interview_change_type": "interview_change_type TEXT",
            "interviewer_names": "interviewer_names TEXT",
            "interview_location": "interview_location TEXT",
            "interview_duration_mins": "interview_duration_mins INTEGER",
        },
    )
    _add_columns_if_missing(
        conn,
        "interviews",
        {
            "rapport_score": "rapport_score INTEGER",
            "role_clarity_score": "role_clarity_score INTEGER",
            "interviewer_engaged_score": "interviewer_engaged_score INTEGER",
            "confidence_score": "confidence_score INTEGER",
            "next_steps_clear": "next_steps_clear INTEGER",
            "timeline_mentioned": "timeline_mentioned INTEGER",
            "compensation_discussed": "compensation_discussed INTEGER",
            "availability_discussed": "availability_discussed INTEGER",
            "debrief_notes": "debrief_notes TEXT",
        },
    )
    _add_columns_if_missing(
        conn,
    ...
    _add_columns_if_missing(
        ...
        "workday_target_health",
        {
            ...
            "last_evaluated": "last_evaluated INTEGER NOT NULL DEFAULT 0",
            "failure_reason": "failure_reason TEXT",
        },
    )
    _add_columns_if_missing(
        conn,
        "generic_target_health",
        {
            "success_count": "success_count INTEGER NOT NULL DEFAULT 0",
            "last_evaluated": "last_evaluated INTEGER NOT NULL DEFAULT 0",
            "failure_reason": "failure_reason TEXT",
...
def get_workday_target_health(conn: sqlite3.Connection, company: str) -> Optional[sqlite3.Row]:
    return conn.execute(
        "SELECT * FROM workday_target_health WHERE company = ?",
        (company,),
    ).fetchone()


def update_workday_target_health(
    conn: sqlite3.Connection,
    company: str,
    careers_url: str,
    status: str,
    ...
    *,
    ...
    elif normalized_status == "cooldown":
...
def get_generic_target_health(conn: sqlite3.Connection, company: str) -> Optional[sqlite3.Row]:
    return conn.execute(
        "SELECT * FROM generic_target_health WHERE company = ?",
        (company,),
    ).fetchone()


def update_generic_target_health(
    conn: sqlite3.Connection,
    company: str,
    careers_url: str,
    status: str,
