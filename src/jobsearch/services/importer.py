...
from jobsearch.ats_db import Opportunity
from .opportunity_service import upsert_job, _safe_str, _safe_float
...
def import_tracker_csv(conn: sqlite3.Connection, csv_path: Path) -> dict:
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            inserted, app_id = upsert_job(conn, opp)
            if inserted:
                summary["inserted"] += 1
            else:
                summary["updated"] += 1

            # Log an interview event if Interview Date is present
            ...
            if interview_date:
                conn.execute(
                    """
                    INSERT INTO events
                        (application_id, event_type, event_date, title, notes, created_at)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (
                        app_id,
